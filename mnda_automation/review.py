...
Review the document against the following standard positions and respond ONLY with valid JSON — no markdown, no explanation.
...
JSON schema:
{{
  "counterparty_name": "Legal entity name from the document, or [COUNTERPARTY NAME] if not found",
  ...
  "findings": [
    {{
      ...
      "redline": "Exact replacement language using preferred language from playbook, or empty string"
    }}
  ],
...
def review_mnda(text, config, counterparty_hint=""):
    ...
    if mode == "keywords":
        return review_with_keywords(text, counterparty_hint)
...
*Status:* {label} | *Reviewed:* {result.review_date} | *Engine:* {result.review_engine}
...
<th style="padding:8px;border:1px solid #e5e7eb;text-align:left;">Redline</th>
</tr></thead>
